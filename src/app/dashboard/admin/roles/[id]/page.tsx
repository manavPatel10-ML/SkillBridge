...
export default function EditRolePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const role = roleDoc.data() as RolePath;
        setFormData({
          title: role.title,
        ...
        // Hydrate selected skills in correct order
        const requiredSkillObjects = role.requiredSkillIds
          .map(id => skillsData.find(s => s.id === id))
          .filter(Boolean) as SkillPreview[];
          
        setSelectedSkills(requiredSkillObjects);
        
      } catch (err) {
        console.error("Failed to fetch data", err);
        setError("Failed to load Role Path.");
      } finally {
        setInitialLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || "Failed to update Role Path.");
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-12">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Edit Role Path</h1>
          <p className="text-gray-600 mt-1">ID: {roleId}</p>
        </div>
      </div>
